import db from "../config/db.js";
import { asyncHandel } from "../middlewares/asyncMiddleware.js";


export const hotelBookingCreate = asyncHandel(async (req, res) => {
    try {

        const { hotel_id, customer_name, customer_phone, check_in_date, passenger, check_out_date, customer_request } = req.body;
        if (req.user.role !== "user") {
            return res.status(403).json({
                success: false,
                message: "Access denied!"
            });
        }
        if (
            !hotel_id ||
            !customer_name ||
            !customer_phone ||
            !check_in_date ||
            !check_out_date
        ) {
            return res.status(400).json({
                success: false,
                message: "All fields are required!"
            });
        }
        const [hotel] = await db.query("SELECT id, shop_id FROM hotels WHERE id = ?", [hotel_id]);
        if (hotel.length === 0) {
            return res.status(404).json({
                success: false,
                message: "Hotel not found!"
            });
        }
        const item = hotel[0]

        const [data] = await db.query(`
            INSERT INTO hotel_bookings  
            (
            user_id,
            shop_id,
            hotel_id,
            customer_name,
            customer_phone,
            check_in_date,
            check_out_date,
            passenger,
            customer_request
            )
            VALUES (?,?,?,?,?,?,?,?,?)
            `, [
            req.user.id,
            item.shop_id,
            item.id,
            customer_name,
            customer_phone,
            check_in_date,
            check_out_date,
            passenger,
            customer_request || null
        ])
        return res.status(201).json({
            success: true,
            message: "Hotel Booking created successfully.",
            hotel_id: data.insertId,
        });

    } catch (error) {
        console.log(error);
        res.status(500).json({
            success: false,
            message: error.message
        });
    }
})

export const hotelBookingList = asyncHandel(async (req, res) => {
    try {

        if (!["admin", "shop"].includes(req.user.role)) {
            return res.status(403).json({
                success: false,
                message: "Access denied!"
            });
        }
        let query = ` 
        SELECT 
        b.id AS booking_id,
        b.user_id,
        b.shop_id,
        b.shop_id,
        
        b.customer_name,
        b.customer_phone,
        DATE_FORMAT(b.check_in_date, '%d-%m-%Y') AS check_in_date,
        DATE_FORMAT(b.check_out_date, '%d-%m-%Y) AS check_out_date,
        b.passenger,
        b.status,
        b.customer_request,

        h.name AS hotel_name,
        h.type,
        h.price,
        h.image,
        h.location,
        h.facilities,
        h.description,

        s.shop_name

        FROM hotel_bookings b JOIN hotels h ON b.hotel_id = h.id
        JOIN shops s ON b.shop_id = s.id 
        `;

        let params = [];
        if (req.user.role === "shop") {
            const [shops] = await db.query(
                "SELECT id FROM shops WHERE user_id = ?",
                [req.user.id]
            );
            if (shops.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: "Shop not found!"
                });
            }
            const shop_id = shops[0].id;
            query += ` WHERE b.shop_id = ?`;
            params.push(shop_id)
        }

        query += ` ORDER BY b.id DESC`
        const [booking] = await db.query(query, params);
        return res.status(200).json({
            success: true,
            message: "Booking List Success",
            booking
        });

    } catch (error) {
        console.log(error);
        res.status(500).json({
            success: false,
            message: error.message
        });
    }
})

export const hotel_bookingApproved = asyncHandel(async (req, res) => {
    try {

        const { id } = req.params;

        if (!["admin", "shop"].includes(req.user.role)) {
            return res.status(403).json({
                success: false,
                message: "Access denied!"
            });
        }

        let shop_id;

        if (req.user.role === "shop") {

            const [shop] = await db.query(
                "SELECT id FROM shops WHERE user_id = ?",
                [req.user.id]
            );

            if (shop.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: "Shop not found!"
                });
            }

            shop_id = shop[0].id;
        }

        let bookingQuery = "SELECT * FROM hotel_bookings WHERE id = ?";
        let bookingParams = [id];

        if (req.user.role === "shop") {
            bookingQuery += " AND shop_id = ?";
            bookingParams.push(shop_id);
        }

        const [booking] = await db.query(
            bookingQuery,
            bookingParams
        );

        if (booking.length === 0) {
            return res.status(404).json({
                success: false,
                message: "Booking not found!"
            });
        }


        const [data] = await db.query("UPDATE hotel_bookings SET status ='approved'  WHERE id = ? AND shop_id = ?", [id, shop_id]);

        return res.status(200).json({
            success: true,
            message: "Hotel Booking Approved Successfully",
            data
        });

    } catch (error) {

        console.log(error);

        res.status(500).json({
            success: false,
            message: error.message
        });
    }
});

export const hotel_bookingCancelled = asyncHandel(async (req, res) => {
    try {

        const { id } = req.params;

        if (!["admin", "shop"].includes(req.user.role)) {
            return res.status(403).json({
                success: false,
                message: "Access denied!"
            });
        }

        let shop_id;

        if (req.user.role === "shop") {

            const [shop] = await db.query(
                "SELECT id FROM shops WHERE user_id = ?",
                [req.user.id]
            );

            if (shop.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: "Shop not found!"
                });
            }

            shop_id = shop[0].id;
        }

        let bookingQuery = "SELECT * FROM hotel_bookings WHERE id = ?";
        let bookingParams = [id];

        if (req.user.role === "shop") {
            bookingQuery += " AND shop_id = ?";
            bookingParams.push(shop_id);
        }

        const [booking] = await db.query(
            bookingQuery,
            bookingParams
        );

        if (booking.length === 0) {
            return res.status(404).json({
                success: false,
                message: "Booking not found!"
            });
        }


        const [data] = await db.query("UPDATE hotel_bookings SET status ='cancelled'  WHERE id = ? AND shop_id = ?", [id, shop_id]);

        return res.status(200).json({
            success: true,
            message: "Hotel Booking Cancelled Successfully",
            data
        });

    } catch (error) {

        console.log(error);

        res.status(500).json({
            success: false,
            message: error.message
        });
    }
});

export const hotelMobileBooking = asyncHandel(async (req, res) => {
    try {
        if (req.user.role !== "user") {
            return res.status(403).json({
                success: false,
                message: "Access denied!"
            });
        }
        const [data] = await db.query(`
        SELECT 
        b.id AS booking_id,
        b.user_id,
        b.shop_id,
        b.shop_id,
        
        b.customer_name,
        b.customer_phone,
        DATE_FORMAT(b.check_in_date, '%d-%m-%Y') AS check_in_date,
        TIME_FORMAT(b.check_out_date, '%h:%i %p') AS check_out_date,
        b.passenger,
        b.status,
        b.customer_request,

       h.name AS hotel_name,
        h.type,
        h.price,
        h.image,
        h.location,
        h.facilities,
        h.description,

        s.shop_name

        FROM restaurant_bookings b

        JOIN restaurants r
        ON b.restaurant_id = r.id

        JOIN shops s
        ON b.shop_id = s.id

        WHERE b.user_id = ?

        ORDER BY b.id DESC
        `, [req.user.id])
        return res.status(200).json({
            success: true,
            message: "Booking Success",
            count: data.length,
            data
        });

    } catch (error) {

        console.log(error);

        res.status(500).json({
            success: false,
            message: error.message
        });
    }
})